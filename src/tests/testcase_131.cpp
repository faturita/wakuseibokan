/* ============================================================================
**
** Scenario 131: Combat.  Walruses fighting each other.
**
...
size_t TestCase_131::addTank(BoxIsland *nemesis, int faction, int walusnumber,GLuint textureId)
{
    // 6,3,12
    ...
    Vec3f p = Vec3f(x, nemesis->getHeight((float)x, (float)z) + 5.0f, z);
    _otter->setPos(p);
    _otter->stop();
    _otter->setSignal(4);
...
void TestCase_131::reset(BoxIsland *nemesis)
{
...
void TestCase_131::checkBeforeDone(unsigned long timertick)
{
    for(int i=0;i<iendpoints;i++)    
    {
...
int TestCase_131::number()
{
    return 131;

}

std::string TestCase_131::title()
{
    return std::string("Combat.  Walruses fighting each other.");
}


bool TestCase_131::done()
{
    return isdone;
}
bool TestCase_131::passed()
{
    return haspassed;
}
std::string TestCase_131::failedMessage()
{
    return message;
}
