...
int TestCase_119::check(unsigned long timertick)
{
    ...
    if (!_w || _w->getHealth() < 1000.0)
    {
        ...
        float estimatedtimemilliseconds = ((100 * 2.0 * sin(33 * PI/180.0))*1000.0)/(9.81);
...
int TestCase_119::number()
{
    return 119;

}

std::string TestCase_119::title()
{
    return std::string("Consistency of target aiming.");
}


bool TestCase_119::done()
{
    return isdone;
}
bool TestCase_119::passed()
{
    return haspassed;
}
std::string TestCase_119::failedMessage()
{
    return message;
}
