...
void Missile::init()
{
    ...
    _model = (Model*)T3DSModel::loadModel(filereader("units/missile.3ds"),130.46696,-5.74,180.85544,1,1,1,0);   // 130 180
    if (_model != NULL)
    {
...
void Missile::drawModel(float yRot, float xRot, float x, float y, float z)
{
    ...
    if (visible)
    {
        ...
        smoke.drawModel(adjustViewLocation(Vehicle::offset,getPos()),getForward());
    }
    //else
    //{
